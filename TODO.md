...
- [ ] **The card→Patterns handoff can hang on "Reading the exact project installed on this card…".** _(agent-runnable)_ Separate from the routing race fixed on `pattern-link-timing-fix`: with `?editPattern=…` preserved and an exact current-project match, the resolve sometimes never settles and the owner sits on a disabled "Verifying project…" button indefinitely — reproduced on `main` as well, roughly one run in two under load, as `tests/card-workspace.spec.ts:1244`. Both card reads inside `readExactCardSnapshot` already carry 3 s timeouts, so the stall is downstream of them (`assertContextCurrent` / the `projectSwitchInFlightRef` error path leaving the reading state on screen). Needs its own diagnosis before the Pattern Lab acceptance run.
