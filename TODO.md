...
- [ ] **Put `footer-build-status.spec.ts` and `setup-ladder.spec.ts` under CI.** _(band: agent-runnable)_ _(effort: quick)_ One line: append both specs to `ci:browser-smoke` in `lightweaver/package.json` (verified 2026-08-20 — the segment then runs 45 tests in 50s, all green). Held back deliberately because `lightweaver/package.json` selects every CI lane by design, so a lone test-wiring edit demands a VERSION bump and publishes a signed firmware release whose binary is unchanged. Carry it on the next firmware-sensitive change instead of paying that price alone. Both specs failed on an untouched `main` for nine days precisely because nothing ran them; PR #148 repaired the fixtures but not the coverage gap.
- [ ] **Audit which Playwright specs no CI lane runs.** _(band: agent-runnable)_ _(effort: moderate)_ Roughly 42 of 57 `lightweaver/tests/*.spec.ts` are in no `ci:*` script — most `layout-*`, `pattern-lab-*`, `setup-*`, plus `studio-route`, `patterns-v3`, `connection-center-quality`. The `live-*` ones are bench-only by design; the rest are the same blind spot that hid the two PR #148 regressions. Decide which are cheap and stable enough to join a lane. Expand the nested `npm run` chains when checking coverage — a plain grep for a filename proves nothing.
