...
- [ ] **Add the 16 verified-green specs to `ci:browser-smoke`.** _(band: agent-runnable)_ _(effort: quick)_ All 16 passed on two consecutive full runs and cost **97s serial**, so the lane goes from 45 tests/50s to roughly 2.5 minutes — still inside the Tests gate budget. In cost order: `pattern-lab-authoring` (24s), `pattern-lab-worker` (13s), `wire-card-capacity` (11s), `pattern-lab-compatibility` (9s), `footer-build-status` (8s), `card-control-drawer`, `layout-mode-switch`, `layout-zoom` (5s each), `setup-ladder`, `pattern-lab-handoff` (3s), `cloud-card-provider`, `setup-adopt-card-project`, `one-owner-per-question`, `pattern-lab-stateful` (2s), `setup-find-card` (1s), `setup-card-reconstruction` (<1s). `modal-navigation` is a 17th candidate on one green run only, since its sole failure was the stale selector repaired the same day. **Carry this on the next firmware-sensitive change** for the same reason PR #149 deferred the earlier two-spec wiring: `lightweaver/package.json` selects every CI lane, so a lone test-wiring edit demands a VERSION bump and publishes a signed firmware release whose binary is unchanged. This entry supersedes the separate `footer-build-status` / `setup-ladder` item — both are in the list above.
- [ ] **Fix the 8 remaining `studio-hardening.spec.ts` failures, then reassess it for a lane.** _(band: agent-runnable)_ _(effort: moderate)_ The spec has rotted unobserved on `main`. Four of its failures were a stale rail selector and are fixed; eight are not, and they cluster on one theme — the card-write / playlist / show acknowledgement paths (`:292`, `:306`, `:326`, `:343`, `:384`, `:400`, `:424`, `:543`). Five are 60s timeouts, which is why the spec alone costs ~6 minutes; it cannot join a lane until they are resolved, and the timeouts make it unclear whether the product or the fixtures moved. The nine other specs carrying single failures are the cheaper follow-ups: `ai-pattern-assistant`, `blocker-fixes`, `card-storage-ui`, `geometry-guide`, `glow`, `panel-resize`, `pattern-lab-isolation` (1 of 8 tests), `pattern-lab-live-preview` (1 of 2).
- [ ] **`live-recording.spec.ts` is broken and misfiled.** _(band: agent-runnable)_ _(effort: quick)_ Unlike its three `live-*` siblings it needs no hardware — it drives localStorage and the Live rail only — so the naming convention is the sole reason nothing runs it. It times out at 60s waiting on `.lw-live-record-meta`. Either repair it and drop the `live-` prefix so it can join a lane, or delete it; leaving a broken spec wearing a bench-only name means nobody will look at it again.
