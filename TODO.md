...
- [x] **Studio and firmware build stamps.** _(agent-runnable)_ Both surfaces now show a monotonic first-parent build number instead of a commit SHA — the Studio footer reads `Studio current · Build 412`, and a card reports `buildNumber` on `/api/firmware-info` so its panel reads `1.0.0 · Build 411`. Published in `/studio-release.json` and the signed firmware manifest, and printed in the deploy workflow summary.
