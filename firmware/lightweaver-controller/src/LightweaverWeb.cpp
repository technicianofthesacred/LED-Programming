...
void handleRoot() {
  ...
  page += F(
            ...
            "const makeConfirmedControl=({initial,render,setDisabled,send,description})=>{let confirmed=initial,activeRequest=0,failed=null,state='idle';const owner={};const request=async value=>{const requestId=++activeRequest;failed=null;state='pending';setDisabled(true);render(value);try{await send(value);if(requestId!==activeRequest)return;confirmed=value;state='confirmed';setDisabled(false);clearControlError(owner)}catch(error){if(requestId!==activeRequest)return;failed=value;state='failed';render(confirmed);setDisabled(false);showControlError('Could not '+description+'. '+((error&&error.message)||'Try again.'),retry,owner)}};const retry=()=>{if(failed===null)return Promise.resolve();const value=failed;clearControlError(owner);return request(value)};const setConfirmed=value=>{activeRequest++;confirmed=value;failed=null;state='confirmed';render(value);setDisabled(false);clearControlError(owner)};const snapshot=()=>({state,confirmed,failed,activeRequest});return{request,retry,setConfirmed,snapshot}};"
