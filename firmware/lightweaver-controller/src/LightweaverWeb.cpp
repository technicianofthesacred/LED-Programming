...
void handleRoot() {
  ...
  page += F(
            ...
            "let hashInstallTail=Promise.resolve();const hashInstallFlights=new Map();"
            "const installFromHash=()=>{try{const sourceHash=location.hash||'';const hash=sourceHash.replace(/^#/,'');if(!hash)return Promise.resolve();const params=new URLSearchParams(hash);const payload=params.get('lwconfig');if(!payload)return Promise.resolve();const existing=hashInstallFlights.get(payload);if(existing)return existing;const job=hashInstallTail.then(async()=>{try{showHandoff('Saving Studio package to this card...');const json=b64urlDecode(payload);const r=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:json});const j=await r.json().catch(()=>({}));if(!r.ok||j.ok===false){showHandoff(j.error||'Could not save Studio package.','err');return}const current=location.hash===sourceHash;if(current)history.replaceState(null,'',location.pathname+location.search);if(j.state==='staged'){showHandoff('New wiring is staged. Return to Studio to run the safe physical test. Your working setup is unchanged.','ok');return}showHandoff('Saved on the card.','ok');if(current&&j.requiresReboot===true&&params.get('reboot')==='1')setTimeout(()=>post('/api/reboot',{}),300)}catch(e){showHandoff(e.message||'Could not read Studio package.','err')}});hashInstallFlights.set(payload,job);hashInstallTail=job.finally(()=>{if(hashInstallFlights.get(payload)===job)hashInstallFlights.delete(payload)});return job}catch(e){showHandoff(e.message||'Could not read Studio package.','err');return Promise.resolve()}};"
