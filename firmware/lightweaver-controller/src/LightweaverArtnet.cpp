...
// Agent A is building LightweaverFrameSource.h/.cpp in parallel. The render
// loop will skip pattern rendering whenever an external source has marked a
// frame recently. We just signal the source enum on every successful Art-Net
// write. If Agent A's header isn't merged yet, we declare a weak forward so
// the link still succeeds and the symbol can be replaced when their object
// file lands. See report — `frameSourceMarkExternal` is the integration seam.
...
#else
extern "C" {
  // Weak stub — Agent A's strong definition will win at link time.
  __attribute__((weak)) void frameSourceMarkExternal(uint8_t /*source*/) {}
}
#endif
...
void decodePacket(const uint8_t* buffer, size_t length) {
  ...
  gTotalFramesRx++;
  frameSourceMarkExternal(uint8_t(FRAME_ARTNET));
}
