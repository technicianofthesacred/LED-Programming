...
test('a connected footer opens customer card controls without a popup', async ({ page }) => {
  ...
  await page.route('http://lightweaver.local/api/zones', route => route.fulfill({ json: {
    zones: [{ id: 'all', label: 'Whole piece', patternId: 'aurora', brightness: 0.7, speed: 1, hueShift: 0, customHue: 32, customSaturation: 230, blackout: false }],
  } }));
  await page.route('http://lightweaver.local/api/patterns', route => route.fulfill({ json: {
    currentId: 'aurora', currentIndex: 0,
    patterns: [{ id: 'aurora', label: 'Aurora', mode: 'procedural', zones: [] }, { id: 'ocean', label: 'Ocean', mode: 'procedural', zones: [] }],
  } }));
  ...
  await page.route('http://lightweaver.local/api/control', async route => {
    ...
    await route.fulfill({ json: { ok: true, cardId: CARD_ID, patternId: controlBody.patternId, revision: controlBody.revision } });
  });
  ...
  await expect(drawer.locator('select[aria-label="Pattern"]')).toHaveValue('aurora');
  await expect(drawer.getByRole('button', { name: 'Blackout' })).toBeVisible();
  ...
  await expect(page).toHaveURL(/\?editPattern=ocean#screen=pattern$/);
