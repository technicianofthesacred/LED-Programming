...
export function WiringAssemblyMap({ wiring, compiled, strips, physicalScale, onClose }) {
  ...
  return (
    <section className="lw-assembly-map" data-testid="wiring-assembly-map">
      <header><div><small>LIGHTWEAVER ASSEMBLY MAP</small><h3>Wiring installation plan</h3><p>{assembly.totalPixels} addressed pixels{assembly.relativeLengths ? ' · jumper lengths are relative' : ''}</p></div><div className="lw-assembly-actions"><button className="btn" title="Open the print dialog for a physical wiring and LED-address reference." data-tooltip="Open the print dialog for a physical wiring and LED-address reference." onClick={() => window.print()}>Print assembly map</button><button className="btn" title="Return to Wire without changing the assembly map." data-tooltip="Return to Wire without changing the assembly map." onClick={onClose}>Close</button></div></header>
