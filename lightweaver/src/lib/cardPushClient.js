...
//
// Hostname resolution: the designer remembers the most recently used host in
// localStorage. Defaults to `lightweaver` which becomes `lightweaver.local`.
...
const HOSTNAME_KEY = 'lw_designer_host';

export function getCardHostname() {
  if (typeof window === 'undefined') return 'lightweaver';
  try {
    return window.localStorage.getItem(HOSTNAME_KEY) || 'lightweaver';
  } catch {
    return 'lightweaver';
  }
}
...
export function setCardHostname(host) {
  ...
  const clean = String(host || 'lightweaver')
    .trim()
    .toLowerCase()
    .replace(/^https?:\/\//, '')
    .replace(/\.local\/?$/, '')
    ...
    .replace(/[^a-z0-9-]/g, '');
  try {
    window.localStorage.setItem(HOSTNAME_KEY, clean || 'lightweaver');
  } catch {
    /* quota — ignore */
  }
}
...
export async function pushConfigToCard(runtimePackage, options = {}) {
  ...
  const url = `http://${host}.local/api/config`;
  ...
  try {
    ...
    if (err && err.name === 'AbortError') {
      throw new CardPushError('offline', `Timed out reaching ${host}.local`, err);
    }
    throw new CardPushError('offline', `Could not reach ${host}.local`, err);
  } finally {
    clearTimeout(timer);
  }
}
