...
export function compileWiring({ wiring, strips = [], groups = [], capabilities = CARD_HARDWARE_CAPABILITIES } = {}) {
  ...
  const empty = { ok: false, errors, warnings, totalPixels: 0, outputs: [], runs: [], pixels: [], zones: [] };
  ...
  return { ok: errors.length === 0, errors, warnings, totalPixels: pixels.length, outputs, runs, pixels, zones, groups };
}
