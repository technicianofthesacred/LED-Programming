...
- [x] Final factory firmware flashed and write-verified on ESP32-S3 card `44:1b:f6:81:fe:b0` through `/dev/cu.usbmodem142201`; boot, stored 44-pixel profile, LAN API, WLED realtime, and Art-Net listeners were verified on 2026-07-17.
- [x] Runtime bench checks passed: external master `100% → 50%` produced requested/applied bytes `166 → 83` without local-look leakage; WLED/Art-Net ownership and the two-second watchdog handed off cleanly; sustained external output reported `128 FPS / dithering on`, returning to `30 FPS / dithering off`; blackout transmitted byte `0`; Recover lights rendered `44/44` neutral white pixels at byte `7` and cancelled streaming.
- [x] Adrian visually confirmed the conservative seven-stage fixture at applied byte `21`: first LED blue/final LED red, full red/green/blue/white, full gray ramp, and low-level gradient all appeared correct. The card was restored to stored Fire playback with neutral output settings and requested/applied byte `166`.
- [ ] Physical non-neutral gamma comparison, per-channel calibration comparison, and sustained full-white current/temperature stress remain installation-specific acceptance items. The card currently has `maxMilliamps: 0`, and its one configured output is mirrored across eight GPIO pins, so those tests must use the final strip/power wiring rather than this neutral 44-pixel fixture run.
- [ ] Phase 2 remains gated only by those installation-specific gamma/calibration/current checks; the Phase 1 neutral output path and live-source correctness are bench-verified.
