...
- Rail renders 7 items: Patterns, Playlist, Layout, **Show**, Flash, Installer, Settings. Both Show AND Settings present; **Show is extra vs the v3 mock** (mock rail had no Show). UNDECIDED whether intended.
- Files still internally labeled "v3": `<title>Lightweaver v3`, "Light Weaver" wordmark, `/* Light Weaver v3 */` headers. Stale naming. UNDECIDED whether to rename.
- node_modules is wiped on container restart — reinstall before running.

## Next
- Decide with Adrian whether to open a NEW PR for `claude/new-ui-branch-status-1Qp4Y` (the CSS fix + this handoff), or fold the CSS fix into other pending work.
- cd /home/user/LED-Programming/lightweaver && npm install && npm run dev   # localhost:5173
- git add -A && git commit && git push -u origin claude/new-ui-branch-status-1Qp4Y   # author must be Claude <noreply@anthropic.com>
- DECIDE: keep **Show** in the rail or remove it to match the v3 mock (rail array in lightweaver/src/v3/app.jsx).
- DECIDE: rename internal "v3" labels to "v3.3" (index.html title + wordmark + lw-*.jsx headers) or leave them.
